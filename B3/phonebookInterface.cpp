...
void executeCommand(const std::string &line, std::ostream &out, Phonebook &phonebook)
{
  ...
  if(command == "add")
  {
    ...
    phonebook.addElement(PhonebookElement{ number, name });
  }
  ...
  else if(command == "insert")
  {
    ...
    if(place == "after")
    {
      phonebook.insertAfter(markName, PhonebookElement{ number,name });
    }
    else
    {
      phonebook.insertBefore(markName, PhonebookElement{ number,name });
    }
  }
  else if(command == "delete")
  {
    std::string markName = readMarkName(stream);
    phonebook.deleteElement(markName);
  }
  else if(command == "show")
  {
    std::string markName = readMarkName(stream);
    phonebook.showElementName(markName, out);
  }
